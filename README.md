...
运行日志按“通道代理 / 通道保活 / 一键准备 / 系统”筛选来源，可叠加级别和关键字搜索；通道来源还可限定首页当前所选通道。一键准备的日志筛选独立于通道选择，准备完成后的保活、停止和异常仍保留原任务标识。每次请求及后台问答沿用开始时的来源和行为，避免状态切换后收尾日志被分错类；文件与界面使用相同标识，例如 `[一键准备][准备 1 · Codex][独立保活][请求 …]`。

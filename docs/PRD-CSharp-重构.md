...
| M7 验收与发布（1–2 天） | 两个 PowerShell 脚本适配并跑通、发布脚本、单文件 EXE、README | `dist\RetryProxy.exe` | **已完成（2026-09-23）**：发布 `build\publish.ps1`（先 `dotnet test -c Release`，可 `-SkipTests`；`dotnet publish -c Release -p:RuntimeIdentifier=win-x64 -p:SelfContained=false -p:PublishSingleFile=true -p:PublishReadyToRun=false -p:DebugType=embedded -o dist`；产物 `dist\RetryProxy.exe` 约 50 MB + `User\I18n` + `runtime-check.cmd`，超过 120 MB 直接报错）。坑：本机 .NET SDK 10.0.201 会忽略 `--self-contained false` 而发布成 220 MB 自包含包，必须写成 `-p:SelfContained=false`；50 MB 中约 26 MB 是 TFM `net9.0-windows10.0.22621.0` 带进来的 `Microsoft.Windows.SDK.NET.dll`，为不改 Debug 输出路径/开发配置保留 TFM。`builduntime-check.cmd` 用 `dotnet --list-runtimes` + findstr 检查 Microsoft.WindowsDesktop.App 9.x 与 Microsoft.AspNetCore.App 9.x。验收脚本移植到 `tests\`：`verify_exe.ps1`（对应 verify_rust_exe.ps1，PowerShell 5.1，`-VerifyTray` 走私有桌面）与 `verify_keepalive.ps1`（对应 verify_keepalive_exe.ps1，pwsh 7，默认/`-Automatic`/`-Interrupt`/`-RetryPreparation`/`-CancelPreparation` 五种模式），共用 `window_verification.cs`（`STARTUPINFO` 不再 SW_HIDE，因为 WPF 会遵守 wShowWindow）。与 Rust 版的语义差异及适配：① 托盘双击不再靠真实鼠标，改为向 WPF-UI 的托盘钩子窗口（标题 `wpfui_th_{主窗口}_{id}`）PostMessage uCallbackMessage 2048 + lParam WM_LBUTTONDBLCLK；② 最小化（SC_MINIMIZE/标题栏按钮/Win+D）留在任务栏（IsIconic），托盘双击或标题栏「最小化到托盘」只切换 Visibility 并保留最大化状态，`NotifyIconViewModel.ShowOrHide` 仅在可见且未最小化时隐藏，否则恢复；③ 导航项是 WPF-UI `NavigationView` 的 DataItem，不暴露 Invoke 模式，脚本用 `SetFocus` + 向主窗口投递 VK_RETURN 切页；④ 「缓存明细」由独立窗口改为页面（D8），检查改为切页后找到「缓存明细 · E2E」标题并回到首页；⑤ 保活脚本改用 `AutomationProperties.Name`（本通道保活 / 保活间隔分钟 / 最小化到托盘）定位控件，通道用 ComboBox 的 ExpandCollapse + SelectionItem 选择，不再点「确定」；⑥ `RETRY_PROXY_CONFIG_JSON` 注入时不落任何 config.json，脚本同时检查根目录与 `User\config.json`；⑦ 窗口恢复三次注入改为每次等到日志多出一条 `WindowRect {..} -> WindowRect {..}` 再注入下一条，应用侧 `WindowRecovery` 若恢复成功要到下一轮轮询才确认也补记「已自动恢复」日志（否则两次注入会并成一条）。运行前提：C# 应用有单实例互斥 `LLM Retry Proxy_SingleInstance`，跑验收前须先 `buildestart-app.ps1 -NoStart` 关掉开发实例，否则测试 EXE 立即退出报 Health endpoint did not start；含中文的 .ps1 必须带 UTF-8 BOM（PowerShell 5.1 否则 Add-Type 解析失败）。结果：`verify_exe.ps1` 普通与 `-VerifyTray`（代理/生成重试/缓存/托盘矩阵/缓存页/隐藏时流式/窗口恢复/关窗退出/两次重启与当日统计）连跑均通过；`verify_keepalive.ps1` 五种模式全部通过；`dotnet test` 270 例通过。README 覆盖运行时要求、目录结构、构建/发布/验收命令、配置与数据位置、环境变量、许可证。 |

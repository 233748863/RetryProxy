...
| M3 Core 缓存与统计（2 天） | PromptCache、兼容重发、ProxyMetrics、DailyJournal、Legacy 恢复 | `prompt_cache` 用例通过；jsonl 互换 | PromptCache、兼容重发、ProxyMetrics 与 prompt_cache 单测已随 M2 落地；剩 DailyJournal（jsonl）、Legacy 日志恢复与 `tests/prompt_cache.rs` 的 9 个集成用例 |
